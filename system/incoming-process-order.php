        ...
        <div class="container-fluid">
            <div class="card rounded-0">
                <div class="card-body login-card-body text-dark">
                    <div class="row">
                        <div class="col-12">
                            <h4 class="float-left text-info">Processed Orders</h4>
                            ...
                            <table id="example1" class="table table-sm table-striped table-hover border border-info">
                                ...
                                <?php
                                    if($country == 'UNITED ARAB EMIRATES') {
                                        ...
                                        trans_status = 'On Process' AND trans_country = '$country' OR 
                                        trans_status = 'In Transit' AND trans_country = '$country' OR 
                                        trans_status = 'On Process' AND trans_country = 'OMAN' OR
                                        trans_status = 'In Transit' AND trans_country = 'OMAN' OR
                                        trans_status = 'On Process' AND trans_country = 'KUWAIT' OR 
                                        trans_status = 'In Transit' AND trans_country = 'KUWAIT' OR
                                        trans_status = 'On Process' AND trans_country = 'BAHRAIN' OR 
                                        trans_status = 'In Transit' AND trans_country = 'BAHRAIN' OR 
                                        trans_status = 'On Process' AND trans_country = 'QATAR' OR
                                        trans_status = 'In Transit' AND trans_country = 'QATAR' 
                                        ORDER BY trans_date ASC";
                                    } elseif ($state == 'ALBERTA' && $country == 'CANADA') {
                                        $order_sql = "SELECT * FROM upti_transaction WHERE 
                                        trans_status= 'On Process' AND trans_country = '$country' AND trans_state = '$state' OR 
                                        trans_status= 'In Transit' AND trans_country = '$country' AND trans_state = '$state' ORDER BY trans_date ASC";
                                    } elseif ($state == 'ALL' && $country == 'CANADA') {
                                        $order_sql = "SELECT * FROM upti_transaction WHERE 
                                        trans_status= 'On Process' AND trans_country = '$country' AND trans_state != 'ALBERTA' OR trans_status= 'In Transit' AND trans_country = '$country' AND trans_state != 'ALBERTA'
                                         ORDER BY trans_date ASC";
                                    } else {
                                        $order_sql = "SELECT * FROM upti_transaction WHERE 
                                        trans_status = 'On Process' AND trans_country = '$country' OR 
                                        trans_status = 'In Transit' AND trans_country = '$country' ORDER BY trans_date ASC";
                                    }
