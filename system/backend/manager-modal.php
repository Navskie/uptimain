<?php
    ...
    if (isset($_POST['manager'])) {
        ...
        if ($get_user_num == 0) {
            $reseller_account = "INSERT INTO upti_manager (manager_name, manager_mobile, manager_address, manager_code, manager_status, manager_date, manager_main) VALUES ('$account_name', '$account_mobile', '$account_address', '$manager_code', 'Active', '$today', '$creator_code')";
            $reseller_account_qry = mysqli_query($connect, $reseller_account);

            $reseller_users = "INSERT INTO upti_users (users_name, users_username, users_password, users_role, users_status, users_code, users_main, users_creator, users_admin) VALUES ('$account_name', '$account_user', '123456', 'UPTIMANAGER', 'Active', '$manager_code', '$creator_code', '$creator_code', '$admin_code1')";
            $reseller_users_qry = mysqli_query($connect, $reseller_users);

            $new_series = $get_series_2 + 1;
            $add_series = "UPDATE upti_series SET series = '$new_series' WHERE remark = 'manager'";
            $add_series_qry = mysqli_query($connect, $add_series);

            echo "<script>alert('Data has been Added successfully.');window.location.href = 'account-manager.php';</script>";
        } else {
            echo "<script>alert('Username Already Exist.');window.location.href = 'account-manager.php';</script>";
        }

    }
?>
<div class="modal fade" id="manager">
    ...
    <div class="modal-content">
        ...
        <button class="btn btn-primary" name="manager">Submit</button>
        </form>
        </div>
